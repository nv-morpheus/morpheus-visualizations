...
export class DeviceBuffers {
  public edge: {
    length: number; id: Buffer; edge: Buffer; color: Buffer; bundle: Buffer; edgeTex: Texture2D;
    bundleTex: Texture2D;
  };
  public icon: {length: number; id: Buffer; age: Buffer; icon: Buffer; edge: Buffer;};
  public node: {
    length: number; id: Buffer; color: Buffer; radius: Buffer; xPositionTex: Texture2D;
...
export class RenderState extends DeviceBuffers {
  ...
  copyRenderMessage(renderMessage: RenderMessage) {
    if (renderMessage.node.changed || (this.node.length !== renderMessage.node.id.length)) {
      this.copyNodeBuffers(renderMessage);
    }
    if (renderMessage.edge.changed || (this.edge.length !== renderMessage.edge.id.length)) {
      this.copyEdgeBuffers(renderMessage);
    }
    if (renderMessage.icon.changed || (this.icon.length !== renderMessage.icon.age.length)) {
      this.copyIconBuffers(renderMessage);
    }
    return this.copyDynamicBuffers(renderMessage);
  }
  ...
  copyDynamicBuffers({bbox, edge, icon, node}: RenderMessage) {
    ...
    this.node.xPositionTex =
      this.copyToFloatTexture2D('RGBA32F', node.xPosition, this.node.xPositionTex);
    this.node.yPositionTex =
      this.copyToFloatTexture2D('RGBA32F', node.yPosition, this.node.yPositionTex);
    return this;
  }
  ...
  copyNodeBuffers({bbox, edge, icon, node}: RenderMessage) {
    ...
    this.node.color =
      this.copyToBuffer(node.color, this.node.color, {size: 4, type: gl.UNSIGNED_BYTE});
    this.node.radius =
      this.copyToBuffer(node.radius, this.node.radius, {size: 1, type: gl.UNSIGNED_BYTE});
    return this;
  }
  ...
  copyEdgeBuffers({bbox, edge, icon, node}: RenderMessage) {
    ...
    this.edge.color =
      this.copyToBuffer(edge.color, this.edge.color, {size: 8, type: gl.UNSIGNED_BYTE});
  ...
  public copyToBuffer(data: TypedArray, buffer?: Buffer|null, accessor: Partial<Accessor> = {}) {
  ...
  public copyToBufferAligned(data: TypedArray,
                             buffer?: Buffer|null,
  ...
  public emptyBufferAligned(length: number,
                            buffer?: Buffer|null,
                            accessor: Partial<Accessor> = {}) {
    accessor  = {size: 1, type: this.gl.FLOAT, ...accessor};
    const BPV = Accessor.getBytesPerVertex(accessor);
  ...
  public emptyFloatTexture2D(format: TextureFormats, size: number, texture?: Texture2D|null) {
    return (texture || this.makeFloatTexture2D(format, new Float32Array()))
      .resize(getTextureSize(format, size * 4, 4));
  }

  public copyToFloatTexture2D(format: TextureFormats,
                              data: Float32Array,
                              texture?: Texture2D|null) {
    if (!texture) { return this.makeFloatTexture2D(format, data); }
