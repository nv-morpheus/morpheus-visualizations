...
export class EdgeLayer extends (Layer as typeof DeckLayer) {
  ...
  static getAccessors({ gl }: { gl: WebGLRenderingContext }) {
    return {
      ...
      instanceEdge: { size: 2, type: gl.INT, accessor: 'getInstanceEdge' },
      instanceBundle: { size: 2, type: gl.INT, accessor: 'getInstanceBundle' },
      instanceSourceColor: { size: 4, stride: 8, offset: 0, type: gl.UNSIGNED_BYTE, accessor: 'getSourceColor' },
      instanceTargetColor: { size: 4, stride: 8, offset: 4, type: gl.UNSIGNED_BYTE, accessor: 'getTargetColor' },
    };
  }
  ...
  getPickingInfo({ mode, info }: { info: PickingInfo, mode: 'hover' | 'click' }) {
    if (info.index === -1) {
      ...
      const { buffer, offset = 0 } = this.props.data.attributes.instanceEdge;
